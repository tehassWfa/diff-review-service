...
// Config is read from env vars, entirely on the server side.
//
//   LLM_API_KEY   - required. Credential for the vendor.
//   LLM_BASE_URL  - optional. Defaults to Anthropic's /v1/messages endpoint.
//   LLM_MODEL     - required. Model name/string to request.
//   LLM_API_STYLE - optional. 'anthropic' (default) or 'openai'. Use 'openai'
//                   for OpenAI itself, or any OpenAI-compatible endpoint such
//                   as Google's Gemini API
...
// OpenAI-compatible chat completions shape. Works with OpenAI itself, and
// with any vendor offering an OpenAI-compatible endpoint (e.g. Google's
// Gemini API at https://generativelanguage.googleapis.com/v1beta/openai/).
async function callOpenAiStyle({ baseUrl, apiKey, model, prompt, timeoutMs }) {
  const controller = new AbortController();
  const timer = setTimeout(() => controller.abort(), timeoutMs);
  try {
    const resp = await fetch(baseUrl, {
...
async function runLlmReview(diffText, maxFindings) {
  ...
  const apiStyle = (process.env.LLM_API_STYLE || 'anthropic').toLowerCase();
  ...
  const caller = apiStyle === 'openai' ? callOpenAiStyle : callAnthropicStyle;

  let responseText;
  try {
    responseText = await caller({ baseUrl, apiKey, model, prompt, timeoutMs });
  } catch (e) {
